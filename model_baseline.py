...
class EncoderLayer(nn.Module):
    def __init__(self, attention, h_dim, heads, dropout=0.1):
        super().__init__()
...
class Encoder(nn.Module):
    def __init__(self, PCA_dim, h_dim, N, heads, attention):
        super().__init__()
        self.N = N
        # self.embed = Embedder(vocab_size, h_dim)
        # self.pe = PositionalEncoder(h_dim)
        self.linear_1 = nn.Linear(PCA_dim, h_dim)
        self.layers = get_clones(EncoderLayer(attention, h_dim, heads), N)
...
class Transformer(nn.Module):
    def __init__(self, seq_len=100, input_dim=100, PCA_dim=128, h_dim=128, N=6, heads=8, attention=Attention,
                 dropout=0.1, cl=6):
        super().__init__()
